...
NOTE: This script was automated to run all 3 tests at once per program call due to time constraints for running Hermes 34b. A more custom version of this test, requiring more human interference would be to remove the "for" loop on tests and manually set those variables
...
def generate_prompt(test):
  # Generate prompt according to the test being run
...
def get_reference_image(test):
  ...
  if "positive" in test:
    if "obvious" in test:
      ...
      return os.path.join(os.path.join(test_files_dir, "positive_img"), "0028.png")
  else:
    return os.path.join(os.path.join(test_files_dir, "negative_img"), "036.png")
...
prompt1 = "This image is from the top of a train. If a flare appears, it is between the train's pantograph and catenary wires.  The first image "
prompt2 = " a flare. Answering with either yes or no, is there a flare in the second image?"
...
test_time = 0
tests = ["negative-", "positive-obvious-", "positive-difficult-"]

for test in tests:
  # For each test, create a new dictionary, prompt and reference
  return_image_dict_list = image_dict_list.copy()
  args.prompt = generate_prompt(test)
  reference = get_reference_image(test)

  for image in tqdm(return_image_dict_list):
    # Run test on each image, record
  ...
  with open(os.path.join(result_file_dir, test + model_string + ".json"), "w") as results_json:
    json.dump(return_image_dict_list, results_json)
